...
class CaptureImages(): 
    ...
    def capture(self, class_name: str) -> bool:     
        try: 
            ...
            # Kamerayı büyüt (örneğin 1.5 kat)
            scale_percent = 150  # yüzde olarak
            width = int(frame.shape[1] * scale_percent / 100)
            height = int(frame.shape[0] * scale_percent / 100)
            dim = (width, height)
            resized_frame = cv2.resize(frame, dim, interpolation=cv2.INTER_LINEAR)

            # Daha küçük font ve ortalanmış yazı
            text = f'Capturing {class_name}'
            font_scale = 1.2
            thickness = 2
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_DUPLEX, font_scale, thickness)
            text_x = (resized_frame.shape[1] - text_size[0]) // 2
            text_y = 50
            image = cv2.putText(resized_frame, text, (text_x, text_y), cv2.FONT_HERSHEY_DUPLEX, font_scale, (0,0,0), thickness, cv2.LINE_AA)
